...
def quaternion_normalize(q, epsilon=1e-7):
    ...
    """
    # 计算四元数的模长，并添加偏移量
    norm = q.norm(p=2, dim=2, keepdim=True) + epsilon
...
class Pipeline_EKF:
    ...
    def rmse(diff: torch.Tensor) -> torch.Tensor:
        """
        Calculates the RMS (Root Mean Square) of the input signal.
        Input:
            diff: torch.Tensor of shape (N, T)
        Output:
            rms: torch.Tensor of shape (N, T)
        """
        # 计算每个元素的平方
        squared = diff ** 2
        
        # 沿时间轴 (T) 计算均值，结果形状为 (N, 1)
    ...
    def NNTrain(self, SysModel, cv_input_acc, cv_input_gyr, cv_input_mag, cv_target, train_input_acc, train_input_gyr, train_input_mag,train_target):
        ...
        for ti in range(0, self.N_steps):
            ...
            #MSE_cvbatch_linear_LOSS = self.angle_error_fn(x_out_cv_batch, cv_target_batch)
            #self.MSE_cv_linear_epoch[ti] = MSE_cvbatch_linear_LOSS.item()
            #print("VQF Train Error:", self.MSE_train_linear_epoch[ti],"[度]","VQF Cv Error:", self.MSE_cv_linear_epoch[ti],"[度]")
