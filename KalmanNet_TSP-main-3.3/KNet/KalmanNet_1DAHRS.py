...
def compute_errors(q: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    """
    计算每个批次的误差向量。

    参数:
        q (torch.Tensor): 输入的四元数张量，形状为 (batch_size, 4)。
        y (torch.Tensor): 加速度计和磁力计拼接的量，形状为 (batch_size, 6)，
                          前三个为加速度计 (ax, ay, az)，后三个为磁力计 (mx, my, mz)。

    返回:
        torch.Tensor: 每个批次的误差向量，形状为 (batch_size, 3)。
    """
    # 提取四元数分量
    q0, q1, q2, q3 = q[:, 0], q[:, 1], q[:, 2], q[:, 3]

    # 提取加速度计和磁力计数据
    ax, ay, az = y[:, 0], y[:, 1], y[:, 2]
    mx, my, mz = y[:, 3], y[:, 4], y[:, 5]
    ...
    # 将误差向量组合成一个张量
    errors = torch.stack([halfex, halfey, halfez], dim=1)
...
class KalmanNetNN(torch.nn.Module):
    ...
    def InitKGainNet(self, prior_Q, prior_Sigma, prior_S, args):
        ...
        self.d_input_S = self.m ** 2  + self.n  * args.in_mult_KNet
    ...
    def step_KGain_est(self, y):
        ...
        obs_innov_diff = compute_errors(self.m1x_prior,y)
    ...
    def KGain_step(self, obs_diff, obs_innov_diff, fw_evol_diff, fw_update_diff):
        ...
        # S-GRU：结合所有特征进行时序建模
        in_S = torch.cat((out_FC1, out_FC8), 2)
