...
def normalize_obs_diff(obs_diff):
    """
    Normalize the first three and last three values of each batch in obs_diff.
    If the norm is zero, return a zero vector for that part to avoid NaN values.

    :param obs_diff: Tensor of shape [batch_size, 6], where each batch contains 6 values.
    :return: Tensor of shape [batch_size, 6], with the first three and last three values normalized.
    """
    # 将 obs_diff 分为前 3 个值和后 3 个值
    first_three = obs_diff[:, :3]  # shape: [batch_size, 3]
    last_three = obs_diff[:, 3:]   # shape: [batch_size, 3]

    # 计算前 3 个值的 2 范数
    norm_first_three = torch.norm(first_three, dim=1, keepdim=True)  # shape: [batch_size, 1]
    norm_last_three = torch.norm(last_three, dim=1, keepdim=True)    # shape: [batch_size, 1]

    # 检查范数是否为零，避免除以零
    mask_first_three = (norm_first_three == 0).float()  # shape: [batch_size, 1], 1 if norm == 0 else 0
    mask_last_three = (norm_last_three == 0).float()    # shape: [batch_size, 1], 1 if norm == 0 else 0

    # 归一化处理：如果范数为零，结果为全零向量
    first_three_normalized = first_three / (norm_first_three + mask_first_three)  # 防止除以零
    last_three_normalized = last_three / (norm_last_three + mask_last_three)     # 防止除以零

    # 使用掩码修正范数为零的情况
    first_three_normalized[mask_first_three.bool().squeeze()] = 0  # 范数为零时置为全零
    last_three_normalized[mask_last_three.bool().squeeze()] = 0    # 范数为零时置为全零

    # 将归一化后的前 3 个值和后 3 个值拼接回原来的形状
    obs_diff_normalized = torch.cat([first_three_normalized, last_three_normalized], dim=1)  # shape: [batch_size, 6]

    return obs_diff_normalized
...
class KalmanNetNN(torch.nn.Module):
    ...
    def step_KGain_est(self, y):
        ...
        #fw_evol_diff = quaternion_normalize(self.m1x_posterior - self.m1x_posterior_previous)#验证集训练出问题
        ...
        #fw_update_diff = quaternion_normalize(self.m1x_posterior -self.m1x_prior_previous)
