...
class KalmanNetNN(torch.nn.Module):
    ...
    def InitKGainNet(self, prior_Q, prior_Sigma, prior_S, args):
        ...
        self.d_input_S = self.m ** 2 + 2 * self.n  * args.in_mult_KNet
        ...
        self.cross_attn = nn.MultiheadAttention(
            embed_dim=self.d_hidden_S + self.d_hidden_Sigma,    # 假设与 GRU_S 的隐藏维度对齐
            num_heads=4
        ).to(self.device)

        # FC2 修改为1D卷积层
        self.d_input_conv_2 = self.d_hidden_S + self.d_hidden_Sigma
        self.d_output_conv_2 = 2 * self.n * self.m 
        ...
        # 输入数据进行特征提取
        # self.d_input_WTC_5 = self.m
        # self.d_output_WTC_5 = self.m 
        # self.input_conv_5 = WTConv2d(self.d_input_WTC_5 , self.d_output_WTC_5, kernel_size=5, wt_levels=2)
        ...
        # fc 6 输入数据进行特征提取
        # self.d_input_WTC_6 = self.m
        # self.d_output_WTC_6 = self.m
        # self.input_conv_6 = WTConv2d(self.d_input_WTC_6 , self.d_output_WTC_6, kernel_size=5, wt_levels=2)
        
        # # fc7 输入数据进行特征提取
        # self.d_input_WTC_7 = 2 * self.n 
        # self.d_output_WTC_7 = 2 * self.n 
        # self.input_conv_7 = WTConv2d(self.d_input_WTC_7, self.d_output_WTC_7, kernel_size=3, wt_levels=1)

        # FC7 修改为1D卷积层
        self.d_input_conv_7 = self.n 
        self.d_output_conv_7 = self.n  * args.in_mult_KNet
        ...
        # FC8 修改为1D卷积层
        self.d_input_conv_8 = self.n 
        self.d_output_conv_8 = self.n  * args.in_mult_KNet
    ...
    def InitSequence(self, batch_size, target_size, y_size):
        ...
        # 初始化测量值 y y(t-1)
        self.m1y = torch.zeros(batch_size, y_size).to(self.device)
    ...
    def KGain_step(self, obs_diff, obs_innov_diff, fw_evol_diff, fw_update_diff):
        ...
        # 将 obs_diff 分为前 3 个值和后 3 个值
        acc_diff = obs_diff[:, :, :3]  # shape: [batch_size, 3]
        mag_diff = obs_diff[:, :, 3:]   # shape: [batch_size, 3]
        ...
        # 数据特征提取：FC5
        # batch_size, features = fw_update_diff.shape
        # in_WTC_5 = fw_update_diff.view(batch_size, features, 1, 1)
        # out_WTC_5 = self.input_conv_5(in_WTC_5)
        # out_WTC_5 = out_WTC_5.view(self.seq_len_input, batch_size, features)
        ...
        # 数据特征提取：FC6
        # batch_size, features = fw_evol_diff.shape
        # in_WTC_6 = fw_evol_diff.view(batch_size, features, 1, 1)
        # out_WTC_6 = self.input_conv_6(in_WTC_6)
        # out_WTC_6 = out_WTC_6.view(self.seq_len_input, batch_size, features)
        ...
        # # 数据特征提取：FC7
        # batch_size, features = obs_diff.shape
        # in_WTC_7 = obs_diff.view(batch_size, features, 1, 1)
        # out_WTC_7 = self.input_conv_7(in_WTC_7)
        # out_WTC_7 = out_WTC_7.view(self.seq_len_input, batch_size, features)
        ...
        # 数据特征提取：FC7
        in_acc = acc_diff.permute(1, 2, 0)  # [batch, features, seq]
        in_mag = mag_diff.permute(1, 2, 0) 
        out_acc = self.conv7(in_acc)
        out_mag = self.conv8(in_mag)
        out_acc = out_acc.permute(2, 0, 1)
        out_mag = out_mag.permute(2, 0, 1)

        # 全连接层 7：结合观测差异和创新差异
        in_FC7 = torch.cat((out_acc, out_mag), 2)
        #in_FC7 = obs_diff
        #in_FC7 = out_WTC_7
        out_FC7 = in_FC7
        ...
        # 全连接层 2：生成更新后的特征
        # 在这里插入多头注意力机制
        in_FC2 = torch.cat((out_Sigma, out_S), 2)  # 拼接out_Sigma和out_S
        # 应用多头注意力机制，query = key = value = 拼接后的特征
        out_FC2_attention, _ = self.cross_attn (in_FC2, in_FC2, in_FC2)
        
        in_conv2 = out_FC2_attention.permute(1, 2, 0)
        out_conv2 = self.conv2(in_conv2)
        out_FC2 = out_conv2.permute(2, 0, 1) 
