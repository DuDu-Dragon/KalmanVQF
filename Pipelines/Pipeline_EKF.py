...
class Pipeline_EKF:
    ...
    def setTrainingParams(self, args):
        ...
        self.N_steps = args.n_steps  # Number of Training Steps 2000
        self.N_B = 1000 # Number of Samples in Batch 1000
    ...
    def loss_fn(self, q1, q2):
        ...
        损失值 (以角度形式)
        """
        # ToDo 2 对四元数进行归一化 [num_batches, 1000, m]
        #对四元数进行归一化 [batch_size, 1000, m]
        q1 = F.normalize(q1, p=2, dim=-1)  # 归一化四元数
        q2 = F.normalize(q2, p=2, dim=-1)  # 归一化四元数
        ...
        # # 计算角度 (cos^-1)，按 element-wise 计算角度
    ...
    def NNTrain(self, SysModel, cv_input_acc, cv_input_gyr, cv_input_mag, cv_target, train_input_acc, train_input_gyr, train_input_mag, train_target):
        ...
        self.MSE_cv_linear_epoch = torch.zeros([self.N_B])
        self.MSE_cv_dB_epoch = torch.zeros([self.N_B])

        self.Loss_train_linear_epoch = torch.zeros([self.N_B])
        self.MSE_train_linear_epoch = torch.zeros([self.N_B])
        ...
        # print("------N_T",N_T)  # ToDo 3 1984
        batch_size = N_T//self.N_B
        ...
        for ti in range(0, self.N_steps):
            ...
            print("--Training Loss:", self.Loss_train_linear_epoch[ti], "[度]")
            self.MSE_train_linear_epoch[ti] = MSE_trainbatch_linear_Metric.item()
            print("  Training MSE:", self.MSE_train_linear_epoch[ti], "[度]")
            # 获取每次迭代的结束时间
            ...
            with torch.no_grad():
                ...
                MSE_cvbatch_linear_Metric = self.loss_fn(x_out_cv_batch, cv_target_batch)
            ...
            if (ti > 1):
                ...
                print("--Diff Training MSE:", d_train, "[度]")
                print("  Diff Validating MSE:", d_cv, "[度]")

            print("--Optimal epoch idx:", self.MSE_cv_idx_opt)
            print("  Optimal MSE:", self.MSE_cv_linear_opt, "[度]")
            # 获取每次迭代的结束时间
